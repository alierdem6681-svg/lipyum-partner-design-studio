...
import PartnerEmbedPanel from "../components/profile/PartnerEmbedPanel.vue";
...
function handleShare(option) {
  shell.openSheet({
    title: option,
    description: "Partner kartı paylaşımı",
    body: `${option} için mock paylaşım akışı hazırlandı.`,
  });
}
</script>
  ...
  <AppPage title="Partner Kartı Önizleme" data-testid="partner-card-preview-page">
    <div class="v-stack v-partner-preview-page">
      ...
      <section class="v-content-list" role="list">
        <AppCard v-for="section in insightSections" :key="section.title" padding="md" as="article" class="v-content-list-item" role="listitem">
      ...
      <AppButton icon="copy" @click="handleShare('Link kopyala')">Linki kopyala</AppButton>
    </div>
  </AppPage>
</template>
