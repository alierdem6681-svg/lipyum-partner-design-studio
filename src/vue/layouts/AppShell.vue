...
import BonusConvertSheet from "../components/wallet/BonusConvertSheet.vue";
...
function completeBonusConvert(result) {
  shell.closeSheet();
  const formatter = new Intl.NumberFormat("tr-TR");
  const message =
    result.mode === "topup"
      ? `${formatter.format(result.totalBalance)} TL bakiye hesabına geçecek.`
      : `${formatter.format(result.cashAmount)} TL bakiye cüzdanına aktarılacak.`;
  ...
  <MobileLayout>
    <div class="v-shell">
      <AppHeader
        ...
        :header-profile="meta.headerProfile"
        :variant="meta.headerVariant"
        :show-back="showBack"
        :right-actions="rightActions"
        :show-default-right-action="meta.showDefaultRightAction"
        @back="goBack"
