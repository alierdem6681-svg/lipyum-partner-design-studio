...
const routesWithoutBottomBar = new Set(["/support/live", "/subscription"]);
...
async function expectCleanVueShell(page) {
  ...
  await expect(page.getByTestId("app-bottom-bar")).toHaveCount(routesWithoutBottomBar.has(route) ? 0 : 1);
...
test("V12-G live support waits then opens branded chat", async ({ page }) => {
  ...
  await expect(page.getByText("Konu: V12-G canli destek")).toBeVisible();
