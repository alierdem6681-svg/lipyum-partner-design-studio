...
test("wallet shows shared credit and bonus summary", async ({ page }) => {
  const errors = await collectConsoleErrors(page);
  ...
  await expect(page.getByTestId("wallet-summary-card")).toBeVisible();
  await expect(page.getByTestId("wallet-bonus-card")).toBeVisible();
