...
test("leaderboard selects, spacing, rewards and score info follow the closed-week contract", async ({ page }) => {
  ...
  await expect(page.getByTestId("leaderboard-top-rankers-card")).toContainText("₺2.500");
  await expect(page.getByTestId("leaderboard-top-rankers-card")).toContainText("₺1.500");
  await expect(page.getByTestId("leaderboard-rewards-card")).toContainText("Haftalık ödül havuzu");
  await expect(page.getByTestId("leaderboard-rewards-card")).toContainText("₺14.000 Bonus");
  await expect(page.getByTestId("leaderboard-rewards-card")).toContainText("Her hafta 14 kişiye 14.000 TL bonus dağıtıyoruz.");
  await expect(page.getByTestId("leaderboard-rewards-card")).toContainText("₺2.500 Bonus");
  await expect(page.getByTestId("leaderboard-rewards-card")).toContainText("₺1.500 Bonus");
  await expect(page.getByTestId("leaderboard-rewards-card")).toContainText("₺700 Bonus");
